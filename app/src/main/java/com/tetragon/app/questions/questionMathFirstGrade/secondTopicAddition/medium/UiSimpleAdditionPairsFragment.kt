...
class UiSimpleAdditionPairsFragment : Fragment(R.layout.fragment_ui_simple_addition_pairs) {
    ...
    private fun generatePairs(root: View) {
        ...
        // First-grade addition: single-digit addends, sums 2..10, all totals distinct
        while (problemData.size < 3) {
            val num1 = Random.nextInt(1, 6) // 1..5
            val num2 = Random.nextInt(1, 6) // 1..5
            val total = num1 + num2

            if (!solutionList.contains(total)) {
    ...
    private fun showFinalSuccessState() {
        ...
        if (isFirstAttempt) {
            activity.totalXp += MathGrade1Type.ADDITION_MATCH.xp
        }
