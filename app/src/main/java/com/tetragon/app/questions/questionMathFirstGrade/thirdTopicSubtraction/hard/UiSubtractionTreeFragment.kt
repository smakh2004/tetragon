...
class UiSubtractionTreeFragment : Fragment(R.layout.fragment_ui_subtraction_tree) {

    // Tree specific views
    private lateinit var wholeNumberText: TextView   // the whole (given, at the top)
    private lateinit var knownPartText: TextView     // the known leaf (given)
    private lateinit var problemAnswerText: TextView // the missing leaf (to fill)
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        wholeNumberText = view.findViewById(R.id.wholeNumber)
        knownPartText = view.findViewById(R.id.firstNumber)
    ...
    private fun setupInstructionText() {
        val fullText = getString(R.string.instruction_subtraction_tree)
    ...
    private fun generateTreeProblem() {
        // Whole is given; one part is given; the child finds the missing part.
        val whole = Random.Default.nextInt(2, 11)         // 2..10
        val knownPart = Random.Default.nextInt(1, whole)  // 1..whole-1
        correctAnswer = whole - knownPart                 // the missing part

        wholeNumberText.text = whole.toString()
        knownPartText.text = knownPart.toString()
        ...
        while (optionSet.size < 3) {
            val distractor = Random.Default.nextInt(1, 11)
    ...
    private fun checkAnswer(index: Int, stateContainer: FrameLayout, circleState: ImageView) {
        ...
        if (chosen == correctAnswer) {
            ...
            if (isFirstAttempt) {
                activity.totalXp += MathGrade1Type.SUBTRACTION_TREE.xp
            }
