 ...
 */
class ConnectNumbersView @JvmOverloads constructor(
    ...
    fun isCorrectSequence(): Boolean {
        if (connected.size != maxNumber) return false
        for (i in connected.indices) if (connected[i].number != i + 1) return false
        return true
    }
    ...
    private fun generateBoard() {
        ...
        val distractorValues = (maxNumber + 1..maxNumber + 8).shuffled().iterator()
        ...
        for (cell in 0 until totalCells) {
            ...
            val c = cell % cols
            val num = cells[cell]
            built.add(Node(if (num > 0) num else distractorValues.next(), r, c))
        }
        nodes = built
        connected.clear()
        layoutNodes()
