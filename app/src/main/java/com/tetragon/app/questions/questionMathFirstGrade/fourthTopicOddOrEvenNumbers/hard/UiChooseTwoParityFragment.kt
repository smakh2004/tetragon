...
class UiChooseTwoParityFragment : Fragment(R.layout.fragment_ui_choose_two_parity) {

    private lateinit var instructionText: TextView
    private lateinit var targetText: TextView

    // Two blanks: [ ] + [ ] = target parity
    ...
    private val SLOT_COUNT = 2
    private val TILE_COUNT = 4

    private var targetIsEven = true
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        initViews(view)
        instructionText.text = getString(R.string.instruction_choose_two)
    ...
    private fun generateProblem() {
        targetIsEven = Random.nextBoolean()
        targetText.text = getString(if (targetIsEven) R.string.label_even else R.string.label_odd)

        // Pool always has 2 evens + 2 odds, so both an even-sum and an odd-sum pair exist.
        val evens = mutableSetOf<Int>()
        while (evens.size < 2) evens.add(listOf(2, 4, 6, 8).random())
        val odds = mutableSetOf<Int>()
        while (odds.size < 2) odds.add(listOf(3, 5, 7, 9).random())

        tileValues = (evens + odds).toList().shuffled()
    ...
    private fun checkAnswer(stateContainer: FrameLayout, circleState: ImageView) {
        ...
        val sum = (slotValue[0] ?: 0) + (slotValue[1] ?: 0)
        val allRight = (sum % 2 == 0) == targetIsEven
        ...
        if (allRight) {
            ...
            if (isFirstAttempt) activity.totalXp += MathGrade1Type.CHOOSE_TWO_PARITY.xp
            activity.handleCorrectAnswer()

            isIncorrectAttempt = false
            checkBtn.text = if (isFinished) getString(R.string.btn_finish) else getString(R.string.btn_continue)
            showCorrectState(stateContainer, circleState, sum)
        } else {
    ...
    private fun colorSlotResult(slotIndex: Int, correctFlag: Boolean) {
        slotButton[slotIndex].setBackgroundResource(
            if (correctFlag) R.drawable.option_btn_top_correct else R.drawable.option_btn_top_incorrect
        )
        slotText[slotIndex].setTextColor(ContextCompat.getColor(requireContext(), R.color.text_color))
    }

    /** Find any valid pair from the pool that matches the target parity. */
    private fun findValidPair(): Pair<Int, Int>? {
        for (i in tileValues.indices) {
            for (j in i + 1 until tileValues.size) {
                val s = tileValues[i] + tileValues[j]
                if ((s % 2 == 0) == targetIsEven) return tileValues[i] to tileValues[j]
            }
        }
        return null
    }

    private fun showCorrectState(stateContainer: FrameLayout, circleState: ImageView, sum: Int) {
        stateContainer.setBackgroundColor(ContextCompat.getColor(requireContext(), R.color.green_3))
        circleState.setImageResource(R.drawable.correct_tick_icon)
        stateAnswer.text = getString(R.string.state_correct)
        answer.visibility = View.VISIBLE
        answer.text = getString(R.string.label_answer, "${slotValue[0]} + ${slotValue[1]} = $sum")
    ...
    private fun setupSeeSolution(stateContainer: FrameLayout, circleState: ImageView) {
        seeEnabledButton.setOnClickListener {
            seeBtn.visibility = View.GONE
            val pair = findValidPair()
            stateAnswer.text = getString(R.string.state_solution)
            if (pair != null) {
                answer.text = getString(R.string.label_answer, "${pair.first} + ${pair.second} = ${pair.first + pair.second}")
                showSlotFilled(0, pair.first)
                showSlotFilled(1, pair.second)
            }
            answer.visibility = View.VISIBLE
