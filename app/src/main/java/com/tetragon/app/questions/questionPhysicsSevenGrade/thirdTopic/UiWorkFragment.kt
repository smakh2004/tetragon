package com.tetragon.app.questions.questionPhysicsSevenGrade.thirdTopic
...
class UiWorkFragment : Fragment(R.layout.fragment_ui_work) {
    ...
    private fun generateProblem() {
        forceValue = (5..25).random()
        distanceValue = (2..10).random()
        correctAnswer = forceValue * distanceValue

        val optionsSet = mutableSetOf(correctAnswer)
        while (optionsSet.size < 3) {
            val wrong = (correctAnswer - 10..correctAnswer + 30).random()
    ...
    private fun checkAnswer(index: Int, stateContainer: FrameLayout, circleState: ImageView) {
        ...
        if (chosenValue == correctAnswer) {
            ...
            if (isFirstAttempt) activity.totalXp += PhysicsGrade7Type.WORK.xp
