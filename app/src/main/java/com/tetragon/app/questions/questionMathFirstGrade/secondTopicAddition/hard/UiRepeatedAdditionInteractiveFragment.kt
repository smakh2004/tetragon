...
class UiRepeatedAdditionInteractiveFragment : Fragment(R.layout.fragment_ui_repeated_addition_interactive) {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        equationText = view.findViewById(R.id.repeatedAddEquationText)
        problemImage = view.findViewById(R.id.repeatedAddAnswerBox)
        problemAnswerText = view.findViewById(R.id.repeatedAddAnswerText)
    ...
    private fun generateProblem() {
        currentProblem = generateRepeatedProblem()
        correctAnswer = evaluateProblem(currentProblem)

        equationText.text = "$currentProblem ="
        problemImage.setImageResource(R.drawable.answer_blue_box)

        // Distractors sit around the correct answer (which can be up to 24 here).
    ...
    // Hard: the same addend repeated four times, e.g. 3+3+3+3, 2+2+2+2.
    private fun generateRepeatedProblem(): String {
        val base = Random.Default.nextInt(2, 7) // 2..6
        val repeatCount = 4
        return List(repeatCount) { base.toString() }.joinToString("+")
    }

    private fun evaluateProblem(problem: String): Int {
        return problem.split("+").sumOf { it.toInt() }
    }
    ...
    private fun checkAnswer(index: Int, stateContainer: FrameLayout, circleState: ImageView) {
        ...
        if (chosen == correctAnswer) {
            ...
            if (isFirstAttempt) {
                activity.totalXp += MathGrade1Type.ADDITION_NUMBERS_REPEATED.xp
            }
