...
class UiNumberOfNaturalNumbersFragment : Fragment(R.layout.fragment_ui_number_of_natural_numbers) {
    ...
    private fun generateProblem() {
        ...
        if (isInclusiveType) {
            ...
            secondNumber = firstNumber + Random.nextInt(10, 25)
            correctAnswer = secondNumber - firstNumber + 1
            val template = getString(R.string.question_natural_numbers_from_to)
            fullText = String.format(template, firstNumber, secondNumber)
        } else {
            firstNumber = Random.nextInt(10, 30)
            secondNumber = firstNumber + Random.nextInt(12, 30)
            correctAnswer = secondNumber - firstNumber - 1
            val template = getString(R.string.question_natural_numbers_between)
            fullText = String.format(template, firstNumber, secondNumber)
        }
    ...
    private fun checkAnswer(index: Int) {
        ...
        if (chosenText == correctAnswer.toString()) {
            ...
            if (isFirstAttempt) {
                activity.totalXp += MathGrade5Type.NUMBER_OF_NATURAL_NUMBERS.xp
            }
            ...
            activity.handleIncorrectAnswer() // FIX: Corrected adaptive difficulty engine mapping
    ...
    private fun setupSeeSolution() {
        seeEnabledButton.setOnClickListener {
            ...
            val solutionExplanation = if (isInclusiveType) {
                "$secondNumber - $firstNumber + 1 = $correctAnswer"
            } else {
                "$secondNumber - $firstNumber - 1 = $correctAnswer"
            }
    ...
    private fun resetFragmentState() {
        val activity = requireActivity() as Math5GradeQuestionActivity
        activity.isCorrectAnswerShowing = false // FIX: Prevents layout/canvas flag leaks into next cycles
