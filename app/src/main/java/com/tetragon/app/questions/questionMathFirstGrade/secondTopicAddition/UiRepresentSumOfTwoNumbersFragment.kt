...
class UiRepresentSumOfTwoNumbersFragment :
    Fragment(R.layout.fragment_ui_represent_sum_of_two_numbers) {
    ...
    private fun generateProblem() {
        val number = Random.nextInt(2, 11)
        ...
        val keywordTwo = getString(R.string.keyword_two_numbers)
        val fullSentence = getString(R.string.instruction_represent_sum_two, targetNumberStr, keywordTwo)
        ...
        val twoIndex = fullSentence.indexOf(keywordTwo)
        if (twoIndex != -1) {
            spannable.setSpan(ForegroundColorSpan(blue), twoIndex, twoIndex + keywordTwo.length, Spannable.SPAN_EXCLUSIVE_EXCLUSIVE)
        }
        ...
        while (finalOptions.size < 3) {
            var fakeSum = Random.nextInt(2, 11)
            while (usedSums.contains(fakeSum)) { fakeSum = Random.nextInt(2, 11) }
    ...
    private fun setupSeeSolution(stateContainer: FrameLayout, circleState: ImageView) {
        seeEnabledButton.setOnClickListener {
            ...
            val targetSum = correctAnswer.first + correctAnswer.second
