...
import com.tetragon.app.databinding.ActivityPhysics11GradeQuestionBinding
...
import com.tetragon.app.questions.questionPhysicsEleventhGrade.firstTopic.UiFindEinsteinFragment
...
class Physics11GradeQuestionActivity : BaseActivity() {
    ...
    private var lastType: PhysicsGrade11Type? = null
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding = ActivityPhysics11GradeQuestionBinding.inflate(layoutInflater)
        ...
        // Logic to get the topic from intent (expected: "MODERN_PHYSICS")
        val topicName = intent.getStringExtra("TOPIC_KEY")
            ?: throw IllegalArgumentException("TOPIC_KEY missing for Grade 11 Physics")

        selectedTopic = PhysicsGrade11Topic.valueOf(topicName)
    ...
    private fun getTypesForTopic(topic: PhysicsGrade11Topic): List<PhysicsGrade11Type> {
        return when (topic) {
            PhysicsGrade11Topic.MODERN_PHYSICS -> listOf(
                PhysicsGrade11Type.EINSTEIN // Example type
            )
        }
    }
    ...
    fun showRandomQuestion() {
        ...
        val fragment = when (nextType) {
            PhysicsGrade11Type.EINSTEIN -> UiFindEinsteinFragment()
        }
