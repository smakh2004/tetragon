package com.tetragon.app.questions.questionMathFirstGrade.fourthTopicOddOrEvenNumbers.easy
...
class UiOddEvenSubtractionFragment : Fragment(R.layout.fragment_ui_odd_even_subtraction) {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        equationText = view.findViewById(R.id.equationText)
        options = listOf(
    ...
    private fun generateProblem() {
        val minuend = Random.Default.nextInt(2, 11)         // 2..10
        val subtrahend = Random.Default.nextInt(1, minuend) // 1..minuend-1
        result = minuend - subtrahend                       // 1..9
        ...
        equationText.text = spannable

        val evenLabel = getString(R.string.label_even)
        val oddLabel = getString(R.string.label_odd)
        correctLabel = if (result % 2 == 0) evenLabel else oddLabel

        // Randomize which button is Even vs Odd so position can't be memorized.
        val labels = listOf(evenLabel, oddLabel).shuffled()
    ...
    private fun checkAnswer(index: Int) {
        ...
        val chosenLabel = (options[index].getChildAt(0) as TextView).text.toString()
        ...
        if (chosenLabel == correctLabel) {
            ...
            if (isFirstAttempt) {
                activity.totalXp += MathGrade1Type.SUBTRACTION_ODD_EVEN.xp
            }
    ...
    private fun setupSeeSolution(stateContainer: FrameLayout, circleState: ImageView) {
        seeEnabledButton.setOnClickListener {
            ...
            // Show both the computed result and its parity word
            answer.text = getString(R.string.label_answer, correctLabel)
            ...
            options.forEach { layout ->
                ...
                layout.setBackgroundResource(
                    if (tv.text.toString() == correctLabel) R.drawable.option_showed else R.drawable.custom_background
                )
            }
        }
    }
