package com.tetragon.app.questions.questionPhysicsSevenGrade.secondTopicDensity
...
class UiDensityFragment : Fragment(R.layout.fragment_ui_density) {
    ...
    private fun generateProblem() {
        volumeValue = (2..5).random()
        massValue = volumeValue * (2..10).random()
        correctAnswer = massValue / volumeValue

        val optionsSet = mutableSetOf(correctAnswer)
        while (optionsSet.size < 3) {
            val wrong = (correctAnswer - 5..correctAnswer + 10).random()
    ...
    private fun checkAnswer(index: Int, stateContainer: FrameLayout, circleState: ImageView) {
        ...
        if (chosenValue == correctAnswer) {
            ...
            if (isFirstAttempt) activity.totalXp += PhysicsGrade7Type.DENSITY.xp
