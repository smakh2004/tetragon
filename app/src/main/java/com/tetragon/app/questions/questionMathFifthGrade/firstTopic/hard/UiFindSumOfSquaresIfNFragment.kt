...
class UiFindSumOfSquaresIfNFragment : Fragment(R.layout.fragment_ui_find_sum_of_squares_if_n) {
    ...
    private fun generateProblem() {
        // Keep N localized to achievable mental ranges for 5th graders
        val samplesN = listOf(5, 6, 7, 8, 10)
        targetN = samplesN[Random.nextInt(samplesN.size)]

        // Sum of squares sequence formula: (n * (n + 1) * (2n + 1)) / 6
        correctAnswer = (targetN * (targetN + 1) * (2 * targetN + 1)) / 6

        val template = getString(R.string.question_sum_of_squares)
        val fullText = String.format(template, targetN, targetN)

        val spannable = SpannableString(fullText)
        val colorBlue = ContextCompat.getColor(requireContext(), R.color.blue_2)

        // Regex matches standard numbers (\d+) AND unicode superscripts/exponents (², ³, etc.)
        val numberAndExponentRegex = Regex("[\\d²³⁴⁵⁶⁷⁸⁹⁰]+")
        numberAndExponentRegex.findAll(fullText).forEach { matchResult ->
            spannable.setSpan(
        ...
        // Distractor 1: Student accidentally calculated standard linear sum instead of squares
        val linearSum = (targetN * (targetN + 1)) / 2
        if (linearSum != correctAnswer && linearSum > 0) distractors.add(linearSum)

        // Distractor 2: Off-by-one computational layout error (missing the final term)
        val missingLastTermSum = correctAnswer - (targetN * targetN)
        if (missingLastTermSum != correctAnswer && missingLastTermSum > 0) distractors.add(missingLastTermSum)

        while (distractors.size < 2) {
            val randomOffset = Random.nextInt(-2, 3) * 5
            val candidate = correctAnswer + randomOffset + Random.nextInt(1, 4)
    ...
    private fun checkAnswer(index: Int) {
        ...
        if (chosenText == correctAnswer.toString()) {
            ...
            if (isFirstAttempt) {
                activity.totalXp += MathGrade5Type.FIND_SUM_OF_SQUARES_IF_N.xp // Fallback layout type reference
            }
    ...
    private fun setupSeeSolution() {
        seeEnabledButton.setOnClickListener {
            ...
            // Define the variables for the formula and the substitution
            val n = targetN
            val formula = "n(n+1)(2n+1) / 6"
            val calculation = "(${n} × ${n + 1} × ${2 * n + 1}) ÷ 6 = $correctAnswer"

            // Combine them into the explanation string
