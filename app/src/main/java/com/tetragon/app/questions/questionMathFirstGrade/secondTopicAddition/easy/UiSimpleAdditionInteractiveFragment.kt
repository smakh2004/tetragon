package com.tetragon.app.questions.questionMathFirstGrade.secondTopicAddition.easy
...
class UiSimpleAdditionInteractiveFragment : Fragment(R.layout.fragment_ui_simple_addition_interactive) {

    private lateinit var simpleAddInteractiveEquationText: TextView
    private lateinit var simpleAddInteractiveRive: RiveAnimationView
    private lateinit var simpleAddInteractiveAnswerText: TextView
    private lateinit var simpleAddInteractiveAnswerBox: ImageView
    ...
    private val checkRunnable = object : Runnable {
        override fun run() {
            if (!isAdded || isAnswerChecked || !::simpleAddInteractiveRive.isInitialized) return
            ...
            if (currentChoice > 0) {
                ...
                if (selectedValue != null) {
                    simpleAddInteractiveAnswerText.text = selectedValue.toString()
                    simpleAddInteractiveAnswerText.visibility = View.VISIBLE
                }
                if (!checkBtn.isEnabled) enableCheckButton()
            } else {
                if (checkBtn.isEnabled) {
                    disableCheckButton()
                    simpleAddInteractiveAnswerText.visibility = View.GONE
                }
            }
    ...
    private fun initViews(view: View) {
        simpleAddInteractiveEquationText = view.findViewById(R.id.simpleAddInteractiveEquationText)
        simpleAddInteractiveAnswerText = view.findViewById(R.id.simpleAddInteractiveAnswerText)
        simpleAddInteractiveAnswerBox = view.findViewById(R.id.simpleAddInteractiveAnswerBox)
        simpleAddInteractiveRive = view.findViewById(R.id.simpleAddInteractiveRive)
    ...
    private fun generateProblem() {
        // Simple first-grade addition: sum between 2 and 10 (e.g. 4 + 5, 3 + 1)
        correctAnswer = Random.nextInt(2, 11)
        val addend1 = Random.nextInt(1, correctAnswer)
        val addend2 = correctAnswer - addend1

        val fullText = "$addend1 + $addend2 ="
        ...
        simpleAddInteractiveEquationText.text = spannable

        // Build 4 unique options in the 1..10 range around the correct answer
        val optionSet = mutableSetOf(correctAnswer)
        while (optionSet.size < 4) {
            val distractor = correctAnswer + Random.nextInt(-3, 4)
            if (distractor != correctAnswer && distractor in 1..10) optionSet.add(distractor)
        }

        val shuffledOptions = optionSet.toList().sorted()
        optionMapping.clear()
        ...
        simpleAddInteractiveAnswerBox.setImageResource(R.drawable.answer_blue_box)
        simpleAddInteractiveAnswerText.visibility = View.GONE
    ...
    private fun checkAnswer() {
        ...
        simpleAddInteractiveRive.setBooleanState(STATE_MACHINE, INPUT_ANSWERED, true)

        if (chosenValue == correctAnswer) {
            playSound(R.raw.correct)
            activity.isCorrectAnswerShowing = true
            activity.playSuccessAnimation()
            simpleAddInteractiveAnswerBox.setImageResource(R.drawable.answer_correct_box)

            val isFinished = activity.incrementProgress()
            if (isFirstAttempt) activity.totalXp += MathGrade1Type.ADDITION_ANIMATION.xp
            ...
            simpleAddInteractiveAnswerBox.setImageResource(R.drawable.answer_incorrect_box)
    ...
    private fun resetForTryAgain() {
        ...
        simpleAddInteractiveRive.setBooleanState(STATE_MACHINE, INPUT_ANSWERED, false)
        simpleAddInteractiveRive.setNumberState(STATE_MACHINE, INPUT_CHOICE, 0f)
    ...
    private fun setupSeeSolution(stateContainer: FrameLayout, circleState: ImageView) {
        seeEnabledButton.setOnClickListener {
            ...
            simpleAddInteractiveAnswerBox.setImageResource(R.drawable.answer_solution_box)
            simpleAddInteractiveAnswerText.text = correctAnswer.toString()
            simpleAddInteractiveAnswerText.visibility = View.VISIBLE
            ...
            correctIdx?.let { simpleAddInteractiveRive.setNumberState(STATE_MACHINE, INPUT_CHOICE, it.toFloat()) }
        }
    }
    ...
    private fun setupRiveViewModel() {
        try {
            val file = simpleAddInteractiveRive.controller.file ?: return
            val vm = file.getViewModelByName("ViewModel1") ?: return
            val vmi = vm.createDefaultInstance()
            simpleAddInteractiveRive.controller.stateMachines.firstOrNull()?.viewModelInstance = vmi
    ...
    private fun getAnswerChoiceValue(): Float {
        var value = 0f
        simpleAddInteractiveRive.controller.stateMachines.firstOrNull()?.inputs?.forEach {
