...
class UiSumOfAllEvenNumbersFragment : Fragment(R.layout.fragment_ui_sum_of_all_even_numbers) {
    ...
    private var isInclusiveType = true
    ...
    private val evenNumbersList = mutableListOf<Int>()
    ...
    private fun generateProblem() {
        isInclusiveType = Random.nextBoolean()
        evenNumbersList.clear()

        val fullText: String
        if (isInclusiveType) {
            firstNumber = Random.nextInt(1, 6)
            secondNumber = firstNumber + Random.nextInt(10, 16)

            for (i in firstNumber..secondNumber) {
                if (i % 2 == 0) evenNumbersList.add(i)
            }
            val template = getString(R.string.question_sum_even_from_to)
            fullText = String.format(template, firstNumber, secondNumber)
        } else {
            firstNumber = Random.nextInt(1, 6)
            secondNumber = firstNumber + Random.nextInt(12, 18)

            for (i in (firstNumber + 1) until secondNumber) {
                if (i % 2 == 0) evenNumbersList.add(i)
            }
            val template = getString(R.string.question_sum_even_between)
            fullText = String.format(template, firstNumber, secondNumber)
        }

        correctAnswer = evenNumbersList.sum()
        val firstNumStr = firstNumber.toString()
        val secondNumStr = secondNumber.toString()

        val spannable = SpannableString(fullText)
        val colorBlue = ContextCompat.getColor(requireContext(), R.color.blue_2)

        val startPosFirst = fullText.indexOf(firstNumStr)
        if (startPosFirst != -1) {
            spannable.setSpan(ForegroundColorSpan(colorBlue), startPosFirst, startPosFirst + firstNumStr.length, Spannable.SPAN_EXCLUSIVE_EXCLUSIVE)
        }

        val startPosSecond = fullText.indexOf(secondNumStr, startIndex = if (startPosFirst != -1) startPosFirst + firstNumStr.length else 0)
        if (startPosSecond != -1) {
            spannable.setSpan(ForegroundColorSpan(colorBlue), startPosSecond, startPosSecond + secondNumStr.length, Spannable.SPAN_EXCLUSIVE_EXCLUSIVE)
        }
        ...
        while (distractors.size < 2) {
            val stepOffset = Random.nextInt(1, 4) * 2
            val direction = if (Random.nextBoolean()) 1 else -1
            val potentialDistractor = correctAnswer + (stepOffset * direction)
            if (potentialDistractor != correctAnswer && potentialDistractor > 0) {
                distractors.add(potentialDistractor)
            }
        }

        val optionsList = (distractors.toList() + correctAnswer).shuffled()
    ...
    private fun checkAnswer(index: Int) {
        ...
        if (chosenText == correctAnswer.toString()) {
            ...
            if (isFirstAttempt) {
                activity.totalXp += MathGrade5Type.SUM_OF_EVEN_NUMBERS.xp
            }
            ...
            activity.handleIncorrectAnswer() // FIX: Mapped engine to follow standard layout progression rules
    ...
    private fun setupSeeSolution() {
        seeEnabledButton.setOnClickListener {
            ...
            val chainExplanation = evenNumbersList.joinToString(separator = " + ")
            val solutionExplanation = "$chainExplanation = $correctAnswer"
    ...
    private fun resetFragmentState() {
        val activity = requireActivity() as Math5GradeQuestionActivity
        activity.isCorrectAnswerShowing = false // FIX: Prevents layout flag leaks into next cycles
