package com.tetragon.app.questions.questionMathFirstGrade.thirdTopicSubtraction.hard
...
class UiRepresentDifferenceOfThreeNumbersFragment :
    Fragment(R.layout.fragment_ui_represent_diffrence_of_three_numbers) {
    ...
    // Triple(a, b, c) meaning a - b - c
    private var correctAnswer: Triple<Int, Int, Int> = Triple(3, 1, 1)
    ...
    /** Build a triple (a, b, c) with a - b - c == target, all parts positive. */
    private fun makeDifferenceTriple(target: Int): Triple<Int, Int, Int> {
        val b = Random.nextInt(1, 4)      // 1..3
        val c = Random.nextInt(1, 4)      // 1..3
        val a = target + b + c            // guarantees a - b - c == target and a - b > 0
        return Triple(a, b, c)
    }

    private fun generateProblem() {
        // The number to represent is the RESULT of a three-term difference.
        val number = Random.nextInt(1, 5) // target 1..4
        correctAnswer = makeDifferenceTriple(number)

        // Localized UI Formatting
        val targetNumberStr = number.toString()
        val keywordThree = getString(R.string.keyword_three_numbers)
        val fullSentence = getString(R.string.instruction_represent_difference, targetNumberStr, keywordThree)
        ...
        // Distractors evaluate to a DIFFERENT result, so only the correct option equals the target.
        val usedResults = mutableSetOf(number)
        val finalOptions = mutableListOf(correctAnswer)

        var guard = 0
        while (finalOptions.size < 3 && guard < 100) {
            guard++
            val fakeResult = Random.nextInt(0, 7) // 0..6
            if (usedResults.contains(fakeResult)) continue
            usedResults.add(fakeResult)
            finalOptions.add(makeDifferenceTriple(fakeResult))
        }
    ...
    /** Evaluate a displayed "a - b - c" string. */
    private fun evaluateDifference(text: String): Int {
        val parts = text.split("-").map { it.trim().toInt() }
    ...
    private fun checkAnswer(index: Int, stateContainer: FrameLayout, circleState: ImageView) {
        ...
        val target = correctAnswer.first - correctAnswer.second - correctAnswer.third
        val isCorrect = evaluateDifference(chosenText) == target
        ...
        if (isCorrect) {
            ...
            if (isFirstAttempt) activity.totalXp += MathGrade1Type.SUBTRACTION_REPRESENTATION.xp
    ...
    private fun setupSeeSolution(stateContainer: FrameLayout, circleState: ImageView) {
        seeEnabledButton.setOnClickListener {
            ...
            options.forEach { layout ->
                val tv = layout.getChildAt(0) as TextView
                layout.setBackgroundResource(
                    if (evaluateDifference(tv.text.toString()) == target) R.drawable.option_showed else R.drawable.custom_background
                )
            }
