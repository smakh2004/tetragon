package com.tetragon.app.questions.questionMathNinthGrade
...
class Math9GradeFragment : Fragment() {
    ...
    // Grade 9 Topics - Localized
    private val topicNames by lazy {
        listOf(getString(R.string.quadratic_equations))
    }

    private val topicKeys = listOf("QUADRATIC_EQUATIONS")

    // Grade 10 Data for Preview - Localized
    private val grade10TopicNames by lazy {
        listOf(getString(R.string.trigonometry))
    }
    ...
    private fun listenToTopicProgress() {
        ...
        topicProgressListener = db.collection("users").document(user.uid).addSnapshotListener { snapshot, _ ->
            ...
            val progressMap = snapshot.get("class9MathProgress") as? Map<*, *> ?: emptyMap<String, Any>()
            val claimedMap = snapshot.get("claimedRewardsMath9") as? Map<*, *> ?: emptyMap<String, Any>()
            ...
            val topicName = grade10TopicNames.getOrNull(activeGrade10Index) ?: grade10TopicNames[0]
            nextTopicName.text = getString(R.string.topic_preview_format, activeGrade10Index + 1, topicName)
    ...
    private fun handleRewardClaimed(view: RiveAnimationView, topicKey: String) {
        val user = auth.currentUser ?: return
        showLoadingState(true)
        db.collection("users").document(user.uid).update("claimedRewardsMath9.$topicKey", true)
