...
class UiCountBackFragment : Fragment(R.layout.fragment_ui_count_back) {
    ...
    private fun generateProblem() {
        // Start somewhere in 3..10, then count back so the result never goes below 1.
        startNumber = Random.Default.nextInt(3, 11)          // 3..10
        countBack = Random.Default.nextInt(1, startNumber)   // 1..startNumber-1
        correctAnswer = startNumber - countBack              // 1..9

        val questionString = getString(R.string.count_back_problem, startNumber, countBack)
        val spannable = SpannableString(questionString)
        val blue = ContextCompat.getColor(requireContext(), R.color.blue_2)

        // Color every run of digits blue.
        Regex("\\d+").findAll(questionString).forEach { match ->
            spannable.setSpan(
        ...
        // Distractors near the correct answer, in the 0..10 range
        val optionSet = mutableSetOf(correctAnswer)
        while (optionSet.size < 3) {
            val fake = correctAnswer + Random.Default.nextInt(-3, 4)
            if (fake != correctAnswer && fake in 0..10) optionSet.add(fake)
        }
    ...
    private fun checkAnswer(index: Int, stateContainer: FrameLayout, circleState: ImageView) {
        ...
        if (chosen == correctAnswer) {
            ...
            if (isFirstAttempt) {
                activity.totalXp += MathGrade1Type.FIND_COUNT_BACK.xp
            }
