...
class UiFindSumOfFirstSomeNaturalFragment : Fragment(R.layout.fragment_ui_find_sum_of_first_some_natural) {
    ...
    private fun generateProblem() {
        // Pick reasonable numbers for 5th graders
        val sampleNumbers = listOf(20, 30, 40, 50, 60, 80, 100)
        targetNumber = sampleNumbers[Random.nextInt(sampleNumbers.size)]

        // Gauss sequence sum formula: (n * (n + 1)) / 2
        correctAnswer = (targetNumber * (targetNumber + 1)) / 2

        val template = getString(R.string.question_sum_of_natural_numbers)
        val fullText = String.format(template, targetNumber)
        val targetNumStr = targetNumber.toString()

        val spannable = SpannableString(fullText)
        val colorBlue = ContextCompat.getColor(requireContext(), R.color.blue_2)

        val startPos = fullText.indexOf(targetNumStr)
        if (startPos != -1) {
            spannable.setSpan(
                ForegroundColorSpan(colorBlue),
                startPos,
                startPos + targetNumStr.length,
        ...
        val distractorShort = (targetNumber * targetNumber) / 2
        if (distractorShort != correctAnswer && distractorShort > 0) distractors.add(distractorShort)
        val distractorNoDivide = targetNumber * (targetNumber + 1)
        if (distractorNoDivide != correctAnswer && distractorNoDivide > 0) distractors.add(distractorNoDivide)

        while (distractors.size < 2) {
            val offset = Random.nextInt(-3, 4) * targetNumber
            val potentialDistractor = correctAnswer + offset + Random.nextInt(1, 5)
            if (potentialDistractor != correctAnswer && potentialDistractor > 0) distractors.add(potentialDistractor)
        }
    ...
    private fun checkAnswer(index: Int) {
        ...
        if (chosenText == correctAnswer.toString()) {
            ...
            if (isFirstAttempt) {
                activity.totalXp += MathGrade5Type.SUM_OF_FIRST_NATURAL_NUMBERS.xp
            }
    ...
    private fun setupSeeSolution() {
        seeEnabledButton.setOnClickListener {
            ...
            val solutionExplanation = "($targetNumber × ${targetNumber + 1}) ÷ 2 = $correctAnswer"
