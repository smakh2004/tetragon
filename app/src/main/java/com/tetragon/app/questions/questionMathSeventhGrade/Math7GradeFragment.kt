package com.tetragon.app.questions.questionMathSeventhGrade
...
class Math7GradeFragment : Fragment() {
    ...
    private lateinit var backToGrade6Btn: View
    ...
    // --- LOCALIZED TOPICS (Grade 7) ---
    private val topicNames by lazy {
        listOf(
            getString(R.string.natural_numbers) // Using your existing "natural_numbers" string
        )
    }
    private val topicKeys = listOf("INTEGERS")

    // --- GRADE 8 DATA FOR DYNAMIC JUMP AHEAD (Localized) ---
    private val grade8TopicNames by lazy {
        listOf(
            getString(R.string.topic_square_roots)
        )
    }
    private val grade8TopicKeys = listOf("SQUARE_ROOTS")
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View {
        return inflater.inflate(R.layout.fragment_math7_grade, container, false)
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        backToGrade6Btn.setOnClickListener {
            val intent = Intent(requireContext(), MainActivity::class.java).apply {
    ...
    private fun initViews(view: View) {
        ...
        backToGrade6Btn = view.findViewById(R.id.back_to_prev_grade_btn)
        ...
        // Use your existing string resource for "8 GRADE"
        nextGradeLabel.text = getString(R.string.eighth_grade)
    }
    ...
    private fun listenToTopicProgress() {
        ...
        topicProgressListener = db.collection("users").document(user.uid).addSnapshotListener { snapshot, _ ->
            ...
            val progressMap = snapshot.get("class7MathProgress") as? Map<*, *> ?: emptyMap<String, Any>()
            val claimedMap = snapshot.get("claimedRewardsMath7") as? Map<*, *> ?: emptyMap<String, Any>()
            ...
            // Dynamic Preview Grade 8
            val progressMap8 = snapshot.get("class8MathProgress") as? Map<*, *> ?: emptyMap<String, Any>()
            val claimedMap8 = snapshot.get("claimedRewardsMath8") as? Map<*, *> ?: emptyMap<String, Any>()
            val activeGrade8Index = findTargetTopicIndex(progressMap8, grade8TopicKeys, claimedMap8)
            val previewName = grade8TopicNames.getOrNull(activeGrade8Index) ?: grade8TopicNames[0]
            nextTopicName.text = getString(R.string.topic_preview_format, activeGrade8Index + 1, previewName)
    ...
    private fun launchQuestionActivity() {
        val intent = Intent(requireContext(), Math7GradeQuestionActivity::class.java).apply {
    ...
    private fun handleRewardClaimed(view: RiveAnimationView, topicKey: String) {
        val user = auth.currentUser ?: return
        showLoadingState(true)
        db.collection("users").document(user.uid).update("claimedRewardsMath7.$topicKey", true)
