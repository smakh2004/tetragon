...
class UiParityOperationFragment : Fragment(R.layout.fragment_ui_parity_operation) {
    ...
    private fun generateProblem() {
        instructionText.text = getString(R.string.instruction_change_parity)

        // Exactly one ODD addend (changes parity) + two even addends (keep parity).
        val oddN = listOf(1, 3, 5).random()
        val evens = listOf(2, 4, 6).shuffled().take(2)
        correctLabel = "+$oddN"

        val labels = (listOf(oddN) + evens).map { "+$it" }.shuffled()
    ...
    private fun checkAnswer(index: Int) {
        ...
        if (chosenLabel == correctLabel) {
            ...
            if (isFirstAttempt) {
                activity.totalXp += MathGrade1Type.PARITY_OPERATION.xp
            }
