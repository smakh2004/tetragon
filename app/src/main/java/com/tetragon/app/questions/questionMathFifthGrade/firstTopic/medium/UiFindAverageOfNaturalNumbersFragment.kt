...
class UiFindAverageOfNaturalNumbersFragment : Fragment(R.layout.fragment_ui_find_average_of_natural_numbers) {
    ...
    private fun generateProblem() {
        ...
        val fullText: String
        if (isInclusiveType) {
            firstNumber = Random.nextInt(1, 25)
            val offset = Random.nextInt(5, 15) * 2
            secondNumber = firstNumber + offset
            correctAnswer = (firstNumber + secondNumber) / 2
            val template = getString(R.string.question_average_numbers_from_to)
            fullText = String.format(template, firstNumber, secondNumber)
        } else {
            firstNumber = Random.nextInt(10, 30)
            val offset = Random.nextInt(4, 12) * 2
            secondNumber = firstNumber + offset
            correctAnswer = (firstNumber + secondNumber) / 2
            val template = getString(R.string.question_average_numbers_between)
            fullText = String.format(template, firstNumber, secondNumber)
        }
        ...
        while (distractors.size < 2) {
            ...
            val potentialDistractor = correctAnswer + offset
            if (potentialDistractor != correctAnswer && potentialDistractor > firstNumber && potentialDistractor < secondNumber) {
    ...
    private fun checkAnswer(index: Int) {
        ...
        if (chosenText == correctAnswer.toString()) {
            ...
            if (isFirstAttempt) {
                activity.totalXp += MathGrade5Type.AVERAGE_OF_NATURAL_NUMBERS.xp
            }
    ...
    private fun setupSeeSolution() {
        seeEnabledButton.setOnClickListener {
            ...
            val solutionExplanation = if (isInclusiveType) {
                "($firstNumber + $secondNumber) / 2 = $correctAnswer"
            } else {
                "(${firstNumber + 1} + ${secondNumber - 1}) / 2 = $correctAnswer"
            }
            ...
            options.forEach { layout ->
                if ((layout.getChildAt(0) as TextView).text.toString() == correctAnswer.toString()) {
    ...
    private fun resetFragmentState() {
        val activity = requireActivity() as Math5GradeQuestionActivity
        activity.isCorrectAnswerShowing = false // FIX: Prevents visual flag leakage
