package com.tetragon.app.questions.questionMathTenthGrade
...
class Math10GradeFragment : Fragment() {
    ...
    private lateinit var backToGrade9Btn: View
    ...
    // --- LOCALIZED TOPICS (Grade 10) ---
    private val topicNames by lazy {
        listOf(
            getString(R.string.trigonometry)
        )
    }
    private val topicKeys = listOf("TRIGONOMETRY")

    // --- GRADE 11 DATA FOR DYNAMIC PREVIEW (Localized) ---
    private val grade11TopicNames by lazy {
        listOf(
            getString(R.string.topic_derivatives)
        )
    }
    private val grade11TopicKeys = listOf("DERIVATIVES")
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View {
        return inflater.inflate(R.layout.fragment_math10_grade, container, false)
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        backToGrade9Btn.setOnClickListener {
            val intent = Intent(requireContext(), MainActivity::class.java).apply {
    ...
    private fun initViews(view: View) {
        ...
        backToGrade9Btn = view.findViewById(R.id.back_to_prev_grade_btn)
    ...
    private fun listenToTopicProgress() {
        ...
        topicProgressListener = db.collection("users").document(user.uid).addSnapshotListener { snapshot, _ ->
            ...
            val progressMap = snapshot.get("class10MathProgress") as? Map<*, *> ?: emptyMap<String, Any>()
            val claimedMap = snapshot.get("claimedRewardsMath10") as? Map<*, *> ?: emptyMap<String, Any>()
            ...
            // Dynamic Preview Grade 11
            val progressMap11 = snapshot.get("class11MathProgress") as? Map<*, *> ?: emptyMap<String, Any>()
            val claimedMap11 = snapshot.get("claimedRewardsMath11") as? Map<*, *> ?: emptyMap<String, Any>()
            val activeGrade11Index = findTargetTopicIndex(progressMap11, grade11TopicKeys, claimedMap11)
            val previewName = grade11TopicNames.getOrNull(activeGrade11Index) ?: grade11TopicNames[0]
            nextTopicName.text = getString(R.string.topic_preview_format, activeGrade11Index + 1, previewName)
    ...
    private fun launchQuestionActivity() {
        val intent = Intent(requireContext(), Math10GradeQuestionActivity::class.java).apply {
    ...
    private fun handleRewardClaimed(view: RiveAnimationView, topicKey: String) {
        val user = auth.currentUser ?: return
        showLoadingState(true)
        db.collection("users").document(user.uid).update("claimedRewardsMath10.$topicKey", true)
    ...
    private fun setupRiveTopics() {
        setupSingleTopic(topic1, R.raw.progress_path_odd, 1f, "TRIGONOMETRY")
    }
        ...
        rive.registerListener(object : RiveFileController.Listener {
            override fun notifyPlay(animation: PlayableInstance) {
                ...
                controller.file?.getViewModelByName("ViewModel1")?.let { vm ->
                    ...
                    // Dynamic text injection for Rive
