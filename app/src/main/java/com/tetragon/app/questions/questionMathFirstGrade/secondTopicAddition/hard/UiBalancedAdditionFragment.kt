package com.tetragon.app.questions.questionMathFirstGrade.secondTopicAddition.hard
...
class UiBalancedAdditionFragment : Fragment(R.layout.fragment_ui_balanced_addition) {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        leftText = view.findViewById(R.id.balanceLeftText)
        rightText = view.findViewById(R.id.balanceRightText)
        problemImage = view.findViewById(R.id.balanceAnswerBox)
        problemAnswerText = view.findViewById(R.id.balanceAnswerText)
    ...
    private fun generateProblem() {
        // Balance / equivalence: a + b = ? + c   (e.g. 12 + 4 = _ + 7)
        // Hard: larger totals. Change 19 back to 11 to cap sums at 10 like the medium version.
        val total = Random.Default.nextInt(5, 19) // sum 5..18
        val a = Random.Default.nextInt(1, total)  // 1..total-1
        val b = total - a
        val c = Random.Default.nextInt(1, total)  // known right-side addend, 1..total-1
        correctAnswer = total - c                 // the missing number

        leftText.text = "$a + $b ="
        rightText.text = "+ $c"
        problemImage.setImageResource(R.drawable.answer_blue_box)

        // Distractors around the missing number, positive and distinct
        val optionSet = mutableSetOf(correctAnswer)
        while (optionSet.size < 3) {
            val distractor = correctAnswer + Random.Default.nextInt(-4, 5)
            if (distractor > 0 && distractor != correctAnswer) optionSet.add(distractor)
        }
        ...
        options.forEachIndexed { index, layout ->
            ...
            tv.text = sortedOptions[index].toString()
    ...
    private fun checkAnswer(index: Int, stateContainer: FrameLayout, circleState: ImageView) {
        ...
        if (chosen == correctAnswer) {
            ...
            if (isFirstAttempt) {
                activity.totalXp += MathGrade1Type.ADDITION_BALANCED.xp
            }
