...
class Physics11GradeFragment : Fragment() {
    ...
    private val topicNames = listOf("Modern Physics")
    private val topicKeys = listOf("MODERN_PHYSICS")
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View {
        return inflater.inflate(R.layout.fragment_physics11_grade, container, false)
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        // Navigation back to Grade 10
        backToPrevGradeBtn.setOnClickListener {
            navigateToGrade(10, "PHYSICS", R.anim.slide_in_left, R.anim.slide_out_right)
        }
    ...
    private fun setupRive() {
        setupSingleTopic(topic1, R.raw.progress_path_odd, 1f, "MODERN_PHYSICS")
    }
    ...
    private fun listenProgress() {
        ...
        listener = db.collection("users").document(user.uid).addSnapshotListener { snap, _ ->
            ...
            val progress = snap.get("class11PhysicsProgress") as? Map<*, *> ?: emptyMap<String, Any>()
            val claimed = snap.get("claimedRewardsPhysics11") as? Map<*, *> ?: emptyMap<String, Any>()
    ...
    private fun handleContinue() {
        ...
        if (pendingRewardKey != null) {
            ...
            val intent = Intent(requireContext(), Physics11GradeQuestionActivity::class.java).apply {
    ...
    private fun claimReward(key: String) {
        val user = auth.currentUser ?: return
        db.collection("users").document(user.uid).update("claimedRewardsPhysics11.$key", true)
