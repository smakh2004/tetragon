package com.example.tetragon.questions.questionMathFourthGrade.fifthTopic
...
class UiMixedNumbersFragment : Fragment(R.layout.fragment_ui_mixed_numbers) {
    ...
    private lateinit var optionWholeTexts: List<TextView>
    private lateinit var optionNumTexts: List<TextView>
    private lateinit var optionDenomTexts: List<TextView>
    private lateinit var optionFracContainers: List<LinearLayout>
    ...
    data class MixedNumberParts(val whole: String, val num: String?, val denom: String?)
    ...
    private fun initViews(view: View) {
        ...
        optionWholeTexts = listOf(view.findViewById(R.id.optionWhole1), view.findViewById(R.id.optionWhole2), view.findViewById(R.id.optionWhole3))
        ...
        optionFracContainers = listOf(view.findViewById(R.id.optionFrac1), view.findViewById(R.id.optionFrac2), view.findViewById(R.id.optionFrac3))
    ...
    private fun generateProblem() {
        ...
        tvWhole1.text = w1.toString(); tvNum1.text = n1.toString(); tvDenom1.text = commonDenom.toString()
        tvWhole2.text = w2.toString(); tvNum2.text = n2.toString(); tvDenom2.text = commonDenom.toString()
        ...
        if (isSubtraction) {
            ...
            val totalNum = n1 + n2
            resW = w1 + w2 + (totalNum / commonDenom)
            resN = totalNum % commonDenom
        }

        correctAnswerString = formatMixedNumber(resW, resN, commonDenom)
        setupOptions()
        resetProblemUI()
    }
    ...
    private fun setupOptions() {
        ...
        while (wrongAnswers.size < 2) {
            val w = (1..6).random()
            val n = (0..5).random()
            val d = listOf(2, 3, 4, 6).random()
            val candidate = formatMixedNumber(w, n, d)
        ...
        optionStrings.forEachIndexed { i, s ->
            val parts = parseString(s)
            optionWholeTexts[i].text = parts.whole
            if (parts.num != null) {
                optionFracContainers[i].visibility = View.VISIBLE
                optionNumTexts[i].text = parts.num
                optionDenomTexts[i].text = parts.denom
            } else {
                optionFracContainers[i].visibility = View.GONE
            }
            options[i].setBackgroundResource(R.drawable.custom_background)
        }
    }

    private fun parseString(s: String): MixedNumberParts {
        return if (s.contains("/")) {
            val mainParts = s.split(" ")
            if (mainParts.size == 2) {
                val frac = mainParts[1].split("/")
                MixedNumberParts(mainParts[0], frac[0], frac[1])
            } else {
                val frac = mainParts[0].split("/")
                MixedNumberParts("", frac[0], frac[1])
            }
        } else {
            MixedNumberParts(s, null, null)
        }
    }
    ...
    private fun formatMixedNumber(w: Int, n: Int, d: Int): String {
        if (n == 0) return w.toString()
        val common = gcd(n, d); val sn = n / common; val sd = d / common
        return if (w == 0) "$sn/$sd" else "$w $sn/$sd"
    }
