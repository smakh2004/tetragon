package com.example.tetragon.questions.questionMathSixthGrade
...
class Math6GradeFragment : Fragment() {
    ...
    private lateinit var backToGrade5Btn: View
    ...
    // --- LOCALIZED TOPICS (Grade 6) ---
    private val topicNames by lazy {
        listOf(
            getString(R.string.fractions) // Replace with your actual R.string.fractions key
        )
    }
    private val topicKeys = listOf("FRACTIONS")

    // --- GRADE 7 DATA FOR DYNAMIC PREVIEW (Localized) ---
    private val grade7TopicNames by lazy {
        listOf(
            getString(R.string.natural_numbers) // Using 7th grade start topic string
        )
    }
    private val grade7TopicKeys = listOf("INTEGERS")
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View {
        return inflater.inflate(R.layout.fragment_math6_grade, container, false)
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        backToGrade5Btn.setOnClickListener {
            val intent = Intent(requireContext(), MainActivity::class.java).apply {
    ...
    private fun initViews(view: View) {
        ...
        backToGrade5Btn = view.findViewById(R.id.back_to_prev_grade_btn)
        ...
        // Localized 7th Grade Label
        nextGradeLabel.text = getString(R.string.seventh_grade)
    }
    ...
    private fun listenToTopicProgress() {
        ...
        topicProgressListener = db.collection("users").document(user.uid).addSnapshotListener { snapshot, _ ->
            ...
            val progressMap = snapshot.get("class6MathProgress") as? Map<*, *> ?: emptyMap<String, Any>()
            val claimedMap = snapshot.get("claimedRewardsMath6") as? Map<*, *> ?: emptyMap<String, Any>()
            ...
            topicKeys.forEachIndexed { index, key ->
                ...
                animateRiveProgress(topicViews[index], currentTopicProgress[key] ?: 0f, prog) { currentTopicProgress[key] = it }
            ...
            // Dynamic Preview Grade 7
            val progressMap7 = snapshot.get("class7MathProgress") as? Map<*, *> ?: emptyMap<String, Any>()
            val claimedMap7 = snapshot.get("claimedRewardsMath7") as? Map<*, *> ?: emptyMap<String, Any>()
            val activeGrade7Index = findTargetTopicIndex(progressMap7, grade7TopicKeys, claimedMap7)
            val previewName = grade7TopicNames.getOrNull(activeGrade7Index) ?: grade7TopicNames[0]
            nextTopicName.text = getString(R.string.topic_preview_format, activeGrade7Index + 1, previewName)
    ...
    private fun launchQuestionActivity() {
        val intent = Intent(requireContext(), Math6GradeQuestionActivity::class.java).apply {
    ...
    private fun handleRewardClaimed(view: RiveAnimationView, topicKey: String) {
        val user = auth.currentUser ?: return
        showLoadingState(true)
        db.collection("users").document(user.uid).update("claimedRewardsMath6.$topicKey", true)
    ...
    private fun setupRiveTopics() {
        setupSingleTopic(topic1, R.raw.progress_path_odd, 1f, "FRACTIONS")
    }
        ...
        rive.registerListener(object : RiveFileController.Listener {
            override fun notifyPlay(animation: PlayableInstance) {
                ...
                controller.file?.getViewModelByName("ViewModel1")?.let { vm ->
                    ...
                    // Inject Localized Text into Rive
