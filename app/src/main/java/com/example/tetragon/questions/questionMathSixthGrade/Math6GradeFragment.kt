...
class Math6GradeFragment : Fragment() {
    ...
    // Grade 6 Topics
    private val topicNames = listOf("Fractions")
    private val topicKeys = listOf("FRACTIONS")

    // --- GRADE 7 DATA FOR DYNAMIC JUMP AHEAD ---
    private val grade7TopicNames = listOf("Integers")
    ...
    private fun listenToTopicProgress() {
        ...
        topicProgressListener = db.collection("users").document(user.uid).addSnapshotListener { snapshot, _ ->
            ...
            topicKeys.forEachIndexed { index, key ->
                val prog = (progressMap6[key] as? Long ?: 0).toFloat()
                val claimed = claimedMap6[key] as? Boolean ?: false
            ...
            currentTargetIndex = findTargetTopicIndex(progressMap6, topicKeys, claimedMap6)

            // 2. DYNAMIC PREVIEW FOR GRADE 7
            ...
            val activeTopicName7 = grade7TopicNames.getOrNull(activeGrade7Index) ?: grade7TopicNames[0]

            nextTopicName.text = "Topic ${activeGrade7Index + 1}: $activeTopicName7"
    ...
    private fun handleRewardClaimed(view: RiveAnimationView, topicKey: String) {
        ...
        db.collection("users").document(user.uid)
            .update("claimedRewardsMath6.$topicKey", true)
    ...
    private fun handleContinueClick() {
        ...
        if (pendingRewardKey != null) {
            ...
            // Updated to target your 6th Grade Question Activity
            ...
            // Reset button after a delay so it's ready when user returns
