...
import com.example.tetragon.databinding.ActivityMath8GradeQuestionBinding
...
class Math8GradeQuestionActivity : BaseActivity() {
    ...
    private fun getTypesForTopic(topic: MathGrade8Topic): List<MathGrade8Type> {
        return when (topic) {
            MathGrade8Topic.SQUARE_ROOTS -> listOf(
                MathGrade8Type.SQUARE_ROOT_ONE
            )
        }
    }
    ...
    fun navigateToXpGained() {
        val intent = Intent(this, XpGainedActivity::class.java).apply {
            ...
            putExtra(SubjectConstants.EXTRA_GRADE, 8) // Updated for Grade 8
