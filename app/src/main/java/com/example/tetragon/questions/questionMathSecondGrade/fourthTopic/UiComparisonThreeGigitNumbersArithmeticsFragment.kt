package com.example.tetragon.questions.questionMathSecondGrade.fourthTopic
...
class UiComparisonThreeGigitNumbersArithmeticsFragment : Fragment(R.layout.fragment_ui_comparison_three_gigit_numbers_arithmetics) {
    ...
    private fun generateProblem() {
        // Generate Left side arithmetic
        val leftData = createArithmeticExpression()
        firstNumber.text = leftData.first
        val leftValue = leftData.second

        // Generate Right side arithmetic
        val rightData = createArithmeticExpression()
        secondNumber.text = rightData.first
        val rightValue = rightData.second
    ...
    private fun createArithmeticExpression(): Pair<String, Int> {
        val isAddition = Random.nextBoolean()
        return if (isAddition) {
            val a = Random.nextInt(100, 500)
            val b = Random.nextInt(10, 499)
            Pair("$a + $b", a + b)
        } else {
    ...
    private fun checkAnswer(index: Int, stateContainer: FrameLayout, circleState: ImageView) {
        ...
        if (chosen == correctAnswer) {
            ...
            if (isFirstAttempt) activity.totalXp += MathGrade2Type.COMPARISON_THREE_DIGITS_ARITHMETICS.xp
