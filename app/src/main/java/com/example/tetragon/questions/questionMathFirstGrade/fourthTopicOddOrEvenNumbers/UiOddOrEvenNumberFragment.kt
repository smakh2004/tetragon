...
class UiOddOrEvenNumberFragment : Fragment(R.layout.fragment_ui_odd_or_even_number) {
    ...
    private fun initViews(view: View) {
        ...
        // Only using two options for this specific question type
        options = listOf(
            view.findViewById(R.id.option1),
            view.findViewById(R.id.option2)
        )
        // Hide third option if it exists in the layout
    ...
    private fun generateProblem() {
        ...
        correctAnswer = if (isEven) "Even" else "Odd"

        val fullText = "Is $number odd or even number?"
        val spannable = SpannableString(fullText)
        val blueColor = ContextCompat.getColor(requireContext(), R.color.blue_2)

        // Highlight the number
        val numStr = number.toString()
        val numStart = fullText.indexOf(numStr)
        spannable.setSpan(ForegroundColorSpan(blueColor), numStart, numStart + numStr.length, Spannable.SPAN_EXCLUSIVE_EXCLUSIVE)

        // Highlight "odd"
        val oddStart = fullText.indexOf("odd")
        spannable.setSpan(ForegroundColorSpan(blueColor), oddStart, oddStart + 3, Spannable.SPAN_EXCLUSIVE_EXCLUSIVE)

        // Highlight "even"
        val evenStart = fullText.indexOf("even")
        spannable.setSpan(ForegroundColorSpan(blueColor), evenStart, evenStart + 4, Spannable.SPAN_EXCLUSIVE_EXCLUSIVE)

        questionText.text = spannable

        // Fixed options: Odd and Even
        val optionTexts = listOf("Odd", "Even")
    ...
    private fun resetUIForNext() {
        val activity = requireActivity() as Math1GradeQuestionActivity
        activity.isResultCurrentlyVisible = false
        activity.hideSuccessAnimation() // Reset the character for the next question
