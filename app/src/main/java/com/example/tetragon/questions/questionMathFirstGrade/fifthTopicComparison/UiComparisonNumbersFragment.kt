...
class UiComparisonNumbersFragment : Fragment(R.layout.fragment_ui_comparison_numbers) {
    ...
    private lateinit var problemAnswerText: TextView // Added variable
    ...
    private fun initViews(view: View) {
        ...
        instructionText = view.findViewById(R.id.instructionText) // Initialize here
    ...
    private fun setupInstructionText() {
        ...
        // Using blue_2 to match your other "missed number" fragments
    ...
    private fun generateProblem() {
        ...
        when (problemType) {
            0 -> { // ADDITION
                leftValue = Random.nextInt(2, 11)
                val a = Random.nextInt(1, leftValue)
                val b = leftValue - a
                firstNumber.text = "$a + $b"

                rightValue = Random.nextInt(2, 11)
                val c = Random.nextInt(1, rightValue)
                val d = rightValue - c
                secondNumber.text = "$c + $d"
            }
            1 -> { // SUBTRACTION
                ...
                leftValue = Random.nextInt(1, 11)
                rightValue = Random.nextInt(1, 11)
    ...
    private fun setupOptionClicks() {
        options.forEachIndexed { index, layout ->
            layout.setOnClickListener {
                ...
                // Update text inside the blue box as the user selects
    ...
    private fun checkAnswer(index: Int, stateContainer: FrameLayout, circleState: ImageView) {
        ...
        if (chosen == correctAnswer) {
            ...
            if (isFirstAttempt) activity.totalXp += MathGrade1Type.COMPARISON.xp

            // 2. Increment the milestone counter
    ...
    private fun setupSeeSolution(stateContainer: FrameLayout, circleState: ImageView, wrongIndex: Int) {
        seeEnabledButton.setOnClickListener {
            ...
            // Show correct answer in the box
            ...
            options.forEachIndexed { _, layout ->
                val tv = layout.getChildAt(0) as TextView
                layout.setBackgroundResource(
                    if (tv.text.toString() == correctAnswer) R.drawable.option_showed else R.drawable.custom_background
                )
            }
            isAnswerChecked = true
            isIncorrectAttempt = false
    ...
    private fun resetForTryAgain() {
        ...
        problemImage.setImageResource(R.drawable.answer_blue_box)
        problemAnswerText.visibility = View.GONE // Hide text for retry
    ...
    private fun resetUIForNext() {
        val activity = requireActivity() as Math1GradeQuestionActivity
        activity.isResultCurrentlyVisible = false
        activity.hideSuccessAnimation() // Clean up character animation
