...
class Math7GradeFragment : Fragment() {
    ...
    // Grade 7 Topics
    private val topicNames = listOf("Integers")
    private val topicKeys = listOf("INTEGERS")

    // --- GRADE 8 DATA FOR DYNAMIC JUMP AHEAD ---
    private val grade8TopicNames = listOf("Square Roots")
    ...
    private fun setupRiveTopics() {
        setupSingleTopic(topic1, R.raw.progress_path_odd, 1f, "INTEGERS")
    }
    ...
    private fun listenToTopicProgress() {
        ...
        topicProgressListener = db.collection("users").document(user.uid).addSnapshotListener { snapshot, _ ->
            ...
            topicKeys.forEachIndexed { index, key ->
                val prog = (progressMap7[key] as? Long ?: 0).toFloat()
                val claimed = claimedMap7[key] as? Boolean ?: false
            ...
            currentTargetIndex = findTargetTopicIndex(progressMap7, topicKeys, claimedMap7)

            // 2. DYNAMIC PREVIEW FOR GRADE 8
    ...
    private fun handleRewardClaimed(view: RiveAnimationView, topicKey: String) {
        ...
        db.collection("users").document(user.uid)
            .update("claimedRewardsMath7.$topicKey", true)
    ...
    private fun handleContinueClick() {
        ...
        if (pendingRewardKey != null) {
            ...
            // Target the 7th Grade Question Activity
