...
class Math10GradeFragment : Fragment() {
    ...
    // Grade 10 Topics
    private val topicNames = listOf("Trigonometry")
    private val topicKeys = listOf("TRIGONOMETRY")

    // --- GRADE 11 DATA FOR DYNAMIC PREVIEW ---
    private val grade11TopicNames = listOf("Derivatives")
    ...
    private fun listenToTopicProgress() {
        ...
        topicProgressListener = db.collection("users").document(user.uid).addSnapshotListener { snapshot, _ ->
            ...
            // 1. GRADE 10 PROGRESS
            val progressMap10 = snapshot.get("class10MathProgress") as? Map<*, *> ?: emptyMap<String, Any>()
            val claimedMap10 = snapshot.get("claimedRewardsMath10") as? Map<*, *> ?: emptyMap<String, Any>()

            var prevClaimed = true
            topicKeys.forEachIndexed { index, key ->
                val prog = (progressMap10[key] as? Long ?: 0).toFloat()
                val claimed = claimedMap10[key] as? Boolean ?: false
            ...
            currentTargetIndex = findTargetTopicIndex(progressMap10, topicKeys, claimedMap10)

            // 2. DYNAMIC PREVIEW FOR GRADE 11
            ...
            val activeTopicName11 = grade11TopicNames.getOrNull(activeGrade11Index) ?: grade11TopicNames[0]

            nextTopicName.text = "Topic ${activeGrade11Index + 1}: $activeTopicName11"
    ...
    private fun handleRewardClaimed(view: RiveAnimationView, topicKey: String) {
        ...
        db.collection("users").document(user.uid)
            .update("claimedRewardsMath10.$topicKey", true)
